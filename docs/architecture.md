...
StructVision Cloud 当前版本面向单节点 GPU Kubernetes 环境，使用等待推理完成后返回的 HTTP 接口。应用 Release 与监控 Release 相互独立，通过 ServiceMonitor、Prometheus 和 Grafana Dashboard ConfigMap 连接业务数据流与可观测性数据流。
...
2. Backend 将上传图片写入 Pod 临时目录，然后通过复用的 `httpx.AsyncClient` 异步上传给 Worker 的 `/predict-file`。
...
- `.Release.Namespace` 将应用 Chart 的资源创建到安装 Release 时选择的 Namespace。本地默认示例使用 `mamt2`，公开镜像部署示例使用 `structvision`。
...
Worker 的资源限制声明 `nvidia.com/gpu: 1`，Kubernetes 按一张 GPU
进行调度。在只有一张 GPU 的节点上，Kubernetes 无法同时调度两个 Worker
Pod。Deployment 因此使用 `Recreate`：旧 Pod 完全退出并释放 GPU 后，才创建
新 Pod。这会产生短暂不可用窗口，但避免 RollingUpdate 因第二个 Pod 无 GPU
可用而停滞。
...
- 当前版本没有对象存储和数据库；Pod 重建会清除上传与结果文件，但不会删除 PVC 模型权重。
